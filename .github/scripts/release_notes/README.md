...
| v5 | (R9 재평가 후 기록) | | | | **운영 첫 릴리스(#75)에서 발견한 결함 2건 반영**: ① 내부 버그픽스를 mobile/required로 오분류 → "서버 결함 수정=other_changes" 규칙 + 골든 R9(forbid_required) 추가 ② 항목 텍스트의 `-->` 인용이 숨은 JSON 주석을 조기 종료시켜 본문에 누출 → base64 임베드로 구조적 차단. +α: notify의 urllib 기본 UA를 Discord(Cloudflare)가 403 차단 → User-Agent 명시 |
